import type { NextPage } from "next";
import {
  Text,
  Center,
  Container,
  VStack,
  Box,
  CircularProgress,
  Input,
  Heading,
  Link,
  Flex
} from "@chakra-ui/react";
import { CheckCircleIcon } from "@chakra-ui/icons";
import { useRouter } from "next/router";
import { useForm, useSubmission } from "../../../hooks";
import { ProofVerificationStatus } from "../../../types";
import { getShortenId, getTxArweaveExplorerUrl } from "../../../utils";

// Get user submissions from Arweave.
const UserSubmission: NextPage = () => {
  const { query } = useRouter();

  // Submission Arweave Tx id
  const submissionTxId = query.submissionId?.toString();
  const { submission } = useSubmission(submissionTxId as string);
  const formId = submission?.formId as string;
  const { form } = useForm(formId);

  if (!submission || !form) {
    return <div>Loading...</div>;
  }

  const { answers, proofsVerified, txId } = submission;
  const { questions } = form;

  return (
    <Center width="100%" display={"flex"} flexDirection="column" p={6}>
      <Container maxW="container.xl">
        <Box>
          <Heading size="md" as="i">
            Submission &nbsp;
            <Link
              href={getTxArweaveExplorerUrl(txId)}
              textDecor="underline"
              isExternal
            >
              {getShortenId(txId)}
            </Link>
          </Heading>
        </Box>
        <Box mt={4}>
          {proofsVerified === ProofVerificationStatus.Verified ? (
            <>
              <Box>
                <CheckCircleIcon color="purple.300"></CheckCircleIcon>
                <Text>Respondent membership verified</Text>
              </Box>
              <Text>Member of group xxx</Text>
            </>
          ) : proofsVerified === ProofVerificationStatus.Nonexistent ? (
            <>
              <Flex alignItems="center">
                <Text>Respondent membership verified</Text>
                &nbsp;
                <CheckCircleIcon color="purple.300"></CheckCircleIcon>
              </Flex>
              <Text>Member of xxx</Text>
            </>
          ) : (
            <CircularProgress
              size={4}
              color="purple.300"
              isIndeterminate
            ></CircularProgress>
          )}
        </Box>

        <VStack spacing={4} align="stretch" mt={4}>
          {questions.map((question, i) => (
            <Box key={i}>
              <Text as="b" fontSize="xl">
                {question.label}
              </Text>
              <Input readOnly variant="flushed" value={answers[i]} />
            </Box>
          ))}
        </VStack>
      </Container>
    </Center>
  );
};

export default UserSubmission;
